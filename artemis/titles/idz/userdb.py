...
class IDZUserDB:
    ...
    def start(self) -> None:
        ...
        asyncio.create_task(
            asyncio.start_server(
                self.connection_cb,
                self.core_config.server.listen_address,
                self.game_config.ports.userdb,
            )
        )
    ...
    def dataReceived(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to decrypt UserDB request from {client_ip} because {e} - {data.hex()}"
            )
        ...
        if magic == 0xFE78571D:
            ...
            self.logger.info(f"Bad UserDB request from from {client_ip}")
            writer.write(b"\x00")
            return
