...
class Mai2Servlet(BaseServlet):
    ...
    def get_routes(self) -> List[Route]:
        return [
            Route(
                "/{version:int}/MaimaiServlet/api/movie/{endpoint:str}",
                self.handle_movie,
                methods=["GET", "POST"],
            ),
            Route(
                "/{version:int}/MaimaiServlet/old/{endpoint:str}", self.handle_old_srv
            ),
            Route(
                "/{version:int}/MaimaiServlet/old/{endpoint:str}/{placeid:str}/{keychip:str}/{userid:int}",
                self.handle_old_srv_userdata,
            ),
            Route(
                "/{version:int}/MaimaiServlet/old/{endpoint:str}/{userid:int}",
                self.handle_old_srv_userdata,
            ),
            Route(
                "/{version:int}/MaimaiServlet/old/{endpoint:str}/{userid:int}",
            ...
            Route(
                "/{version:int}/MaimaiServlet/usbdl/{endpoint:str}", self.handle_usbdl
            ),
            Route(
                "/{version:int}/MaimaiServlet/deliver/{endpoint:str}",
                self.handle_deliver,
            ),
            Route(
                "/{version:int}/MaimaiServlet/{endpoint:str}",
                self.handle_mai,
                methods=["POST"],
            ),
            Route(
                "/{version:int}/Maimai2Servlet/{endpoint:str}",
                self.handle_mai2,
                methods=["POST"],
            ),
        ]
    ...
    async def handle_mai(self, request: Request) -> bytes:
        ...
        return Response(
            zlib.compress(json.dumps(resp, ensure_ascii=False).encode("utf-8"))
        )
