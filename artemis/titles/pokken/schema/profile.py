...
class PokkenProfileData(BaseData):
    ...
    async def update_profile_tutorial_flags(
        self, user_id: int, tutorial_flags: List
    ) -> None:
        sql = (
            update(profile)
            .where(profile.c.user == user_id)
    ...
    async def update_profile_achievement_flags(
    ...
    async def update_profile_event(
        self,
        user_id: int,
        event_state: List,
    ...
    async def add_pokemon_xp(self, user_id: int, pokemon_id: int, xp: int) -> None:
        sql = (
            update(pokemon_data)
            .where(
                and_(
            ...
            .values(pokemon_exp=coalesce(pokemon_data.c.pokemon_exp, 0) + xp)
        )
    ...
    async def get_pokemon_data(self, user_id: int, pokemon_id: int) -> Optional[Row]:
        sql = pokemon_data.select(
            and_(pokemon_data.c.user == user_id, pokemon_data.c.char_id == pokemon_id)
        )
        result = await self.execute(sql)
        if result is None:
            return None
    ...
    async def put_pokemon_battle_result(
        ...
        sql = (
            ...
            .values(
                battle_num_tutorial=coalesce(pokemon_data.c.battle_num_tutorial, 0) + 1
                if match_type == PokkenConstants.BATTLE_TYPE.TUTORIAL
                else coalesce(pokemon_data.c.battle_num_tutorial, 0),
                battle_all_num_tutorial=coalesce(
                    pokemon_data.c.battle_all_num_tutorial, 0
                )
                + 1
                if match_type == PokkenConstants.BATTLE_TYPE.TUTORIAL
                else coalesce(pokemon_data.c.battle_all_num_tutorial, 0),
                battle_num_vs_cpu=coalesce(pokemon_data.c.battle_num_vs_cpu, 0) + 1
                if match_type == PokkenConstants.BATTLE_TYPE.AI
                else coalesce(pokemon_data.c.battle_num_vs_cpu, 0),
                win_cpu=coalesce(pokemon_data.c.win_cpu, 0) + 1
                if match_type == PokkenConstants.BATTLE_TYPE.AI
                and match_result == PokkenConstants.BATTLE_RESULT.WIN
                else coalesce(pokemon_data.c.win_cpu, 0),
                battle_num_vs_lan=coalesce(pokemon_data.c.battle_num_vs_lan, 0) + 1
                if match_type == PokkenConstants.BATTLE_TYPE.LAN
                else coalesce(pokemon_data.c.battle_num_vs_lan, 0),
                win_vs_lan=coalesce(pokemon_data.c.win_vs_lan, 0) + 1
                if match_type == PokkenConstants.BATTLE_TYPE.LAN
                and match_result == PokkenConstants.BATTLE_RESULT.WIN
                else coalesce(pokemon_data.c.win_vs_lan, 0),
                battle_num_vs_wan=coalesce(pokemon_data.c.battle_num_vs_wan, 0) + 1
                if match_type == PokkenConstants.BATTLE_TYPE.WAN
                else coalesce(pokemon_data.c.battle_num_vs_wan, 0),
                win_vs_wan=coalesce(pokemon_data.c.win_vs_wan, 0) + 1
                if match_type == PokkenConstants.BATTLE_TYPE.WAN
                and match_result == PokkenConstants.BATTLE_RESULT.WIN
                else coalesce(pokemon_data.c.win_vs_wan, 0),
            )
        )
    ...
    async def update_support_team(
        self, user_id: int, support_id: int, support1: int = None, support2: int = None
    ) -> None:
        sql = (
            update(profile)
            .where(profile.c.user == user_id)
