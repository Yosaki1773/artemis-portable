...
class PokkenBase:
    ...
    async def handle_save_user(self, request: jackal_pb2.Request) -> bytes:
        ...
        if (
            ...
            await self.data.profile.set_profile_name(
                user_id,
        ...
        await self.data.profile.update_profile_event(
            user_id, evt_state, evt_flgs, evt_params, req.last_play_event_id
        )

        for reward in req.reward_data:
            await self.data.item.add_reward(
                user_id,
        ...
        await self.data.profile.add_profile_points(
            user_id, get_rank_pts, get_money, get_score_pts, grade_max
        )

        await self.data.profile.update_support_team(
        ...
        await self.data.profile.put_pokemon(
            user_id,
            mon.char_id,
        ...
        await self.data.profile.add_pokemon_xp(
            user_id, mon.char_id, mon.get_pokemon_exp
        )
    ...
    async def handle_admission_noop(
        self, data: Dict, req_ip: str = "127.0.0.1"
    ) -> Dict:
        return {}

    async def handle_admission_joinsession(
        self, data: Dict, req_ip: str = "127.0.0.1"
    ) -> Dict:
