...
class SaoGetMaintResponse(SaoBaseResponse):
    ...
    def make(self) -> bytes:
        ...
        resp_data = (
            struct.pack("!bI", self.result, self.maint_begin_int_ct)
            + maint_begin_bytes
            + struct.pack("!I", self.maint_end_int_ct)
            + maint_end_bytes
        )
...
class SaoHomeCheckAcLoginBonusResponse(SaoBaseResponse):
    def __init__(self, cmd) -> None:
        ...
        self.get_ac_login_bonus_id_2 = (
            2  # "2020年10月6日～（秋のデビュー＆カムバックCP）"
        )
...
class SaoGetHeroLogUserDataListResponse(SaoBaseResponse):
    ...
    def make(self) -> bytes:
        ...
        resp_data["hero_log_user_data_list_size"] = len(
            resp_data.hero_log_user_data_list
        )
...
class SaoGetEquipmentUserDataListResponse(SaoBaseResponse):
    ...
    def make(self) -> bytes:
        ...
        resp_data["equipment_user_data_list_size"] = len(
            resp_data.equipment_user_data_list
        )
...
class SaoGetSupportLogUserDataListResponse(SaoBaseResponse):
    ...
    def make(self) -> bytes:
        ...
        resp_data["support_log_user_data_list_size"] = len(
            resp_data.support_log_user_data_list
        )
...
class SaoGetEpisodeAppendDataListResponse(SaoBaseResponse):
    ...
    def make(self) -> bytes:
        ...
        resp_data["episode_append_data_list_size"] = len(
            resp_data.episode_append_data_list
        )
...
class SaoEpisodePlayEndUnanalyzedLogFixedResponse(SaoBaseResponse):
    ...
    def make(self) -> bytes:
        ...
        resp_data["play_end_unanalyzed_log_reward_data_list_size"] = len(
            resp_data.play_end_unanalyzed_log_reward_data_list
        )
        for i in range(len(resp_data.play_end_unanalyzed_log_reward_data_list)):
            resp_data.play_end_unanalyzed_log_reward_data_list[i][
                "common_reward_data_size"
            ] = len(
                resp_data.play_end_unanalyzed_log_reward_data_list[i][
                    "common_reward_data"
...
class SaoGetQuestSceneUserDataListResponse(SaoBaseResponse):
    ...
    def make(self) -> bytes:
        ...
        resp_data["quest_scene_user_data_list_size"] = len(
            resp_data.quest_scene_user_data_list
        )
        for i in range(len(resp_data.quest_scene_user_data_list)):
            resp_data.quest_scene_user_data_list[i][
                "quest_scene_best_score_user_data_size"
            ] = len(
                resp_data.quest_scene_user_data_list[i][
                    "quest_scene_best_score_user_data"
...
class SaoSynthesizeEnhancementHeroLogRequest(SaoBaseRequest):
    def __init__(self, header: SaoRequestHeader, data: bytes) -> None:
        ...
        self.material_common_reward_user_data_list: List[
            MaterialCommonRewardUserData
        ] = []
...
class SaoSynthesizeEnhancementEquipmentResponse(SaoBaseResponse):
    ...
    def make(self) -> bytes:
        ...
        resp_data["after_equipment_user_data_size"] = len(
            resp_data.after_equipment_user_data
        )
...
class SaoGetQuestHierarchyProgressDegreesRankingListResponse(SaoBaseResponse):
    def __init__(self, cmd_id: int) -> None:
        super().__init__(cmd_id)
        self.result = 1  # byte
        self.quest_hierarchy_progress_degrees_ranking_data_list: List[
            QuestHierarchyProgressDegreesRankingData
        ] = []
...
class SaoGetQuestPopularHeroLogRankingListResponse(SaoBaseResponse):
    def __init__(self, cmd_id: int) -> None:
        super().__init__(cmd_id)
        self.result = 1  # byte
        self.quest_popular_hero_log_ranking_data_list: List[
            PopularHeroLogRankingData
        ] = []
