...
class SaoBase:
    ...
    async def handle_c12a(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        self.logger.info(
            f"Give {req.give_num} free tickets (id {req.ticket_id}) to user {req.user_id}"
        )
    ...
    async def handle_c11e(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        if not user_id:
            ...
            await self.data.item.put_hero_log(
                user_id, 101000010, 1, 0, 201000000, 0, 1002, 1003, 1014, 30001, 30310
            )
            await self.data.item.put_hero_log(
                user_id, 102000010, 1, 0, 202000000, 0, 3001, 3002, 3004, 30007, 3011
            )
            await self.data.item.put_hero_log(
                user_id,
                105000010,
            ...
            await self.data.item.put_hero_log(
                user_id, 101000110, 1, 0, 201000000, 101000110, 2002, 2001, 2014, 0, 0
            )
            await self.data.item.put_hero_party(
                user_id, 0, 101000010, 102000010, 105000010
            )
    ...
    async def handle_c804(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        resp = SaoGetPartyDataListResponse(
            header.cmd + 1, hero1_data, hero2_data, hero3_data
        )
        return resp.make()

    async def handle_c902(
        self, header: SaoRequestHeader, request: bytes
    ) -> (
        bytes
    ):  # for whatever reason, having all entries empty or filled changes nothing
    ...
    async def handle_c814(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        synthesize_hero_log_data = await self.data.item.get_hero_log(
            req.user_id, req.origin_user_hero_log_id
        )
        ...
        for x in req.material_common_reward_user_data_list:
            ...
            equipmentList = await self.data.static.get_equipment_id(
                x.user_common_reward_id
            )
            ...
            if equipmentList:
                equipment_data = await self.data.item.get_user_equipment(
                    req.user_id, x.user_common_reward_id
                )
                if equipment_data is None:
                    self.logger.error(
                        f"Failed to find equipment {x.user_common_reward_id} for user {req.user_id}!"
                    )
                    continue
                ...
                await self.data.item.remove_equipment(
                    req.user_id, x.user_common_reward_id
                )

            if heroList:
                hero_data = await self.data.item.get_hero_log(
                    req.user_id, x.user_common_reward_id
                )
                if hero_data is None:
                    self.logger.error(
                        f"Failed to find hero {x.user_common_reward_id} for user {req.user_id}!"
                    )
                    continue
                ...
                await self.data.item.remove_hero_log(
                    req.user_id, x.user_common_reward_id
                )

            if hero_exp == 0:
                self.logger.warn(
                    f"Hero {x.user_common_reward_id} (type {x.common_reward_type}) not found!"
                )
        ...
        resp = SaoSynthesizeEnhancementHeroLogResponse(
            header.cmd + 1, synthesize_hero_log_data
        )
        return resp.make()
    ...
    async def handle_c816(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        synthesize_equipment_data = await self.data.item.get_user_equipment(
        ...
        for x in req_data.material_common_reward_user_data_list:
            ...
            if itemList:
                ...
                await self.data.item.remove_item(
                    req_data.user_id, x.user_common_reward_id
                )
            ...
            if equipmentList:
                ...
                if equipment_data is None:
                    self.logger.error(
                        f"Failed to find equipment {x.user_common_reward_id} for user {req_data.user_id}!"
                    )
                    continue
            ...
            if heroList:
                ...
                if hero_data is None:
                    self.logger.error(
                        f"Failed to find hero {x.user_common_reward_id} for user {req_data.user_id}!"
                    )
                    continue
            ...
            if equipment_exp == 0:
                self.logger.warn(
                    f"Common reward {x.user_common_reward_id} (type {x.common_reward_type}) not found!"
                )
                continue

            await self.data.item.put_equipment_data(
                req_data.user_id,
        ...
        resp = SaoSynthesizeEnhancementEquipmentResponse(
            header.cmd + 1, synthesize_equipment_data
        )
        return resp.make()
    ...
    async def handle_c806(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        for party_team in req_data.party_data_list[0].party_team_data_list:
            hero_data = await self.data.item.get_hero_log(
                req_data.user_id, party_team.user_hero_log_id
            )
            hero_level = 1
            hero_exp = 0
    ...
    async def handle_c908(
        ...
        quest_clear_flag = bool(
            req_data.play_end_request_data_list[0]
            .score_data_list[0]
            .boss_destroying_num
        )
        clear_time = (
            req_data.play_end_request_data_list[0].score_data_list[0].clear_time
        )
        combo_num = req_data.play_end_request_data_list[0].score_data_list[0].combo_num
        total_damage = (
            req_data.play_end_request_data_list[0].score_data_list[0].total_damage
        )
        ...
        col = int(profile["own_col"]) + int(
            req_data.play_end_request_data_list[0].base_get_data_list[0].get_col
        )
        ...
        session_party = await self.data.item.get_hero_party(
        ...
        for r in range(
            0, req_data.play_end_request_data_list[0].get_rare_drop_data_count
        ):
            rewardList = await self.data.static.get_rare_drop_id(
                int(
                    req_data.play_end_request_data_list[0]
                    .get_rare_drop_data_list[r]
                    .quest_rare_drop_id
                )
            )
        ...
        for a in range(
            0,
            req_data.play_end_request_data_list[
                0
            ].get_unanalyzed_log_tmp_reward_data_count,
        ):
            ...
            heroList = await self.data.static.get_hero_id(
                randomized_unanalyzed_id["CommonRewardId"]
            )
            equipmentList = await self.data.static.get_equipment_id(
                randomized_unanalyzed_id["CommonRewardId"]
            )
            itemList = await self.data.static.get_item_id(
                randomized_unanalyzed_id["CommonRewardId"]
            )
            if heroList:
                await self.data.item.put_hero_log(
        ...
        await self.data.item.create_end_session(
    ...
    async def handle_c90a(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        resp = SaoEpisodePlayEndUnanalyzedLogFixedResponse(
            header.cmd + 1, end_session_data[4]
        )
        return resp.make()

    async def handle_c91a(
        self, header: SaoRequestHeader, request: bytes
    ) -> bytes:  # handler is identical to the episode
    ...
    async def handle_d5da(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        for shop in shops:
            tmp = YuiMedalShopData.from_args(
                int(shop["YuiMedalShopId"]), shop["Name"], shop["Description"]
            )
    ...
    async def handle_d5dc(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        for shop in shops:
            tmp = YuiMedalShopItemData.from_args(
                int(shop["YuiMedalShopItemId"]),
                int(shop["YuiMedalShopId"]),
                int(shop["CommonRewardType"]),
                int(shop["CommonRewardId"]),
                int(shop["CommonRewardNum"]),
                int(shop["Strength"]),
            )
    ...
    async def handle_d5fc(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        for shop in shops:
            tmp = GashaMedalShop.from_args(
                int(shop["GashaMedalShopId"]),
                shop["Name"],
                int(shop["GashaMedalId"]),
                int(shop["UseGashaMedalNum"]),
                int(shop["PurchaseLimit"]),
            )
            tmp.sales_end_date = datetime(2121, 1, 1, 0, 0, 0, 0)  # always open
    ...
    async def handle_d604(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        for shop in shops:
            tmp = ResEarnCampaignShop.from_args(
                int(shop["ResEarnCampaignShopId"]),
                int(shop["ResEarnCampaignApplicationId"]),
                shop["Name"],
            )
            tmp.selling_yui_medal = int(shop["SellingYuiMedal"])
            tmp.selling_col = int(shop["SellingCol"])
    ...
    async def handle_cb02(self, header: SaoRequestHeader, request: bytes) -> bytes:
        ...
        return SaoGetQuestHierarchyProgressDegreesRankingListResponse(
            header.cmd + 1
        ).make()
