...
class IDACServlet(BaseServlet):
    ...
    def get_routes(self) -> List[Route]:
        return [
            Route(
                "/{version:int}/initiald/{category:str}/{endpoint:str}",
                self.render_POST,
                methods=["POST"],
            ),
            Route(
                "/{version:int}/initiald-matching/{endpoint:str}",
    ...
    def get_allnet_info(
        ...
        t_port = (
            f":{title_port_int}"
            if title_port_int and not self.core_cfg.server.is_using_proxy
            else ""
        )
    ...
    async def render_matching(self, request: Request):
        ...
        self.logger.info(f"IDAC Matching request from {client_ip}: {url} - {req_data}")
        ...
        if url == "/regist":
            ...
            if req_data.get("cancel_flag"):
                self.queue = self.queue - 1
                self.logger.info(f"IDAC Matching endpoint {client_ip} had quited")
        ...
        if self.game_cfg.server.enable:
            ...
            asyncio.create_task(
                loop.create_datagram_endpoint(
                    lambda: IDACEchoUDP(),
                    local_addr=(
                        self.core_cfg.server.listen_address,
            ...
            self.logger.info(
                f"Matching listening on {self.game_cfg.server.matching} with echo on {self.game_cfg.server.echo1}"
            )
