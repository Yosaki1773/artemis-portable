...
from .handlers.base import BaseRequest, BaseResponse
...
class WaccaServlet(BaseServlet):
    ...
    def get_routes(self) -> List[Route]:
        return [
            Route(
                "/WaccaServlet/api/{api:str}/{endpoint:str}",
                self.render_POST,
                methods=["POST"],
            ),
            Route(
                "/WaccaServlet/api/{api:str}/{branch:str}/{endpoint:str}",
                self.render_POST,
                methods=["POST"],
            ),
        ]
    ...
    def get_allnet_info(
        ...
        return (
            f"http://{self.core_cfg.server.hostname}/WaccaServlet",
            self.core_cfg.server.hostname,
        )
    ...
    async def render_POST(self, request: Request) -> bytes:
        ...
        try:
            ...
            self.logger.error(f"Failed to parse request to {url_path} -> {bod} -> {e}")
            resp = BaseResponse()
            resp.status = 1
            resp.message = "不正なリクエスト エラーです"
            return end(resp.make())
        ...
        try:
            ...
            if self.logger.level == logging.DEBUG:
                ...
                with open(
                    "{0}/{1}.log".format(self.core_cfg.server.log_dir, "wacca"), "a"
                ) as f:
