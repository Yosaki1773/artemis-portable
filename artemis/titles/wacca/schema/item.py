...
from sqlalchemy import Column, Table, UniqueConstraint, and_, case
...
class WaccaItemData(BaseData):
    ...
    async def unlock_song(
        self, user_id: int, song_id: int, difficulty: int
    ) -> Optional[int]:
    ...
    async def put_item(
        self, user_id: int, item_type: int, item_id: int
    ) -> Optional[int]:
