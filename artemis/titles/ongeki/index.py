...
class OngekiServlet(BaseServlet):
    ...
    def get_routes(self) -> List[Route]:
        return [
            Route(
                "/SDDT/{version:int}/{endpoint:str}", self.render_POST, methods=["POST"]
            )
        ]
    ...
    def get_allnet_info(
        ...
        proto = (
            "https" if self.game_cfg.server.use_https and game_ver >= 120 else "http"
        )
    ...
    async def render_POST(self, request: Request) -> bytes:
        ...
        if not encrtped and self.game_cfg.crypto.encrypted_only and version >= 120:
            self.logger.error(
