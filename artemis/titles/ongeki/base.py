...
class OngekiBase:
    ...
    async def handle_upsert_client_setting_api_request(self, data: Dict) -> Dict:
        ...
        if cab is not None:
            await self.data.static.put_client_setting_data(
                cab["id"], client_setting_data
            )
        return {"returnCode": 1, "apiName": "UpsertClientSettingApi"}
    ...
    async def handle_get_user_tech_event_api_request(self, data: Dict) -> Dict:
        user_tech_event_list = await self.data.item.get_tech_event(
            self.version, data["userId"]
        )
    ...
    async def handle_get_user_tech_event_ranking_api_request(self, data: Dict) -> Dict:
        user_tech_event_ranks = await self.data.item.get_tech_event_ranking(
            self.version, data["userId"]
        )
    ...
    async def handle_get_user_event_ranking_api_request(self, data: Dict) -> Dict:
        user_event_ranking_list = await self.data.item.get_ranking_event_ranks(
            self.version, data["userId"]
        )
    ...
    async def handle_get_user_recent_rating_api_request(self, data: Dict) -> Dict:
        recent_rating = await self.data.profile.get_profile_recent_rating(
            data["userId"]
        )
    ...
    async def handle_get_user_activity_api_request(self, data: Dict) -> Dict:
        activity = await self.data.profile.get_profile_activity(
    ...
    async def handle_get_user_training_room_by_key_api_request(
        self, data: Dict
    ) -> Dict:
        return {
            "userId": data["userId"],
    ...
    async def handle_get_user_mission_point_api_request(self, data: Dict) -> Dict:
        user_mission_point_list = await self.data.item.get_mission_points(
            self.version, data["userId"]
        )
    ...
    async def handle_upsert_user_all_api_request(self, data: Dict) -> Dict:
        ...
        if "userOption" in upsert and len(upsert["userOption"]) > 0:
            await self.data.profile.put_profile_options(
        ...
        if "userBpBaseList" in upsert:
            await self.data.profile.put_profile_bp_list(
                user_id, upsert["userBpBaseList"]
            )
