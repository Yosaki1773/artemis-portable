...
class OngekiProfileData(BaseData):
    ...
    async def put_profile_data(
        self, aime_id: int, version: int, data: Dict
    ) -> Optional[int]:
        data["user"] = aime_id
    ...
    async def put_profile_options(
        self, aime_id: int, options_data: Dict
    ) -> Optional[int]:
    ...
    async def put_profile_region(
        self, aime_id: int, region: int, date: str
    ) -> Optional[int]:
        sql = insert(activity).values(
    ...
    async def put_training_room(self, aime_id: int, room_detail: Dict) -> Optional[int]:
        ...
        if result is None:
            self.logger.warning(
                f"put_best_score: Failed to add score! aime_id: {aime_id}"
            )
            return None
        return result.lastrowid
    ...
    async def delete_rival(self, aime_id: int, rival_id: int) -> Optional[int]:
        sql = delete(rival).where(
            rival.c.user == aime_id, rival.c.rivalUserId == rival_id
        )
        result = await self.execute(sql)
        if result is None:
            self.logger.error(
                f"delete_rival: failed to delete! aime_id: {aime_id}, rival_id: {rival_id}"
            )
        else:
