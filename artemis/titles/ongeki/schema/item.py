...
class OngekiItemData(BaseData):
    ...
    async def get_item(
        self, aime_id: int, item_id: int, item_kind: int
    ) -> Optional[Dict]:
    ...
    async def get_items(
        self, aime_id: int, item_kind: int = None
    ) -> Optional[List[Dict]]:
    ...
    async def put_music_item(
        self, aime_id: int, music_item_data: Dict
    ) -> Optional[int]:
    ...
    async def put_login_bonus(
        self, aime_id: int, login_bonus_data: Dict
    ) -> Optional[int]:
        ...
        if result is None:
            self.logger.warning(
                f"put_login_bonus: Failed to update! aime_id: {aime_id}"
            )
            return None
        return result.lastrowid
    ...
    async def put_mission_point(
        self, aime_id: int, version: int, mission_point_data: Dict
    ) -> Optional[int]:
        ...
        if result is None:
            self.logger.warning(
                f"put_mission_point: Failed to update! aime_id: {aime_id}"
            )
            return None
        return result.lastrowid

    async def get_mission_points(
        self, version: int, aime_id: int
    ) -> Optional[List[Dict]]:
        sql = select(mission_point).where(
            and_(mission_point.c.user == aime_id, mission_point.c.version == version)
        )
        result = await self.execute(sql)
    ...
    async def put_event_point(
        self, aime_id: int, version: int, event_point_data: Dict
    ) -> Optional[int]:
        ...
        if result is None:
            self.logger.warning(
                f"put_event_point: Failed to update! aime_id: {aime_id}"
            )
            return None
        return result.lastrowid
    ...
    async def put_trade_item(
        self, aime_id: int, trade_item_data: Dict
    ) -> Optional[int]:
    ...
    async def put_event_music(
        self, aime_id: int, event_music_data: Dict
    ) -> Optional[int]:
        ...
        if result is None:
            self.logger.warning(
                f"put_event_music: Failed to update! aime_id: {aime_id}"
            )
            return None
        return result.lastrowid
    ...
    async def put_tech_event(
        self, aime_id: int, version: int, tech_event_data: Dict
    ) -> Optional[int]:
    ...
    async def put_tech_event_ranking(
        self, aime_id: int, version: int, tech_event_data: Dict
    ) -> Optional[int]:
        tech_event_data["user"] = aime_id
        tech_event_data["version"] = version
        ...
        if result is None:
            self.logger.warning(
                f"put_tech_event_ranking: Failed to update ranking! aime_id {aime_id}"
            )
            return None
        return result.lastrowid

    async def get_tech_event(self, version: int, aime_id: int) -> Optional[List[Dict]]:
        sql = select(tech_event).where(
            and_(tech_event.c.user == aime_id, tech_event.c.version == version)
        )
        result = await self.execute(sql)
    ...
    async def put_memorychapter(
        ...
        if result is None:
            self.logger.warning(
                f"put_memorychapter: Failed to update! aime_id: {aime_id}"
            )
            return None
        return result.lastrowid
    ...
    async def put_user_gacha(
        self, aime_id: int, gacha_id: int, **data
    ) -> Optional[int]:
    ...
    async def get_ranking_event_ranks(
        self, version: int, aime_id: int
    ) -> Optional[List[Dict]]:
        ...
        sql = select(
            event_point.c.id,
            event_point.c.user,
            event_point.c.eventId,
            event_point.c.type,
            func.row_number()
            .over(
                partition_by=event_point.c.eventId, order_by=event_point.c.point.desc()
            )
            .label("rank"),
            event_point.c.date,
            event_point.c.point,
        ).where(event_point.c.version == version)
        result = await self.execute(sql)
        if result is None:
            self.logger.error(
                f"failed to rank aime_id: {aime_id} ranking event positions"
            )
            return None
        return result.fetchall()

    async def get_tech_event_ranking(
        self, version: int, aime_id: int
    ) -> Optional[List[Dict]]:
        sql = select(
            tech_ranking.c.id,
            tech_ranking.c.user,
            tech_ranking.c.date,
            tech_ranking.c.eventId,
            func.row_number()
            .over(
                partition_by=tech_ranking.c.eventId,
                order_by=[
                    tech_ranking.c.totalTechScore.desc(),
                    tech_ranking.c.totalPlatinumScore.desc(),
                ],
            )
            .label("rank"),
            tech_ranking.c.totalTechScore,
            tech_ranking.c.totalPlatinumScore,
        ).where(tech_ranking.c.version == version)
        result = await self.execute(sql)
        if result is None:
