...
class OngekiFrontend(FE_Base):
    ...
    async def render_GET(self, request: Request) -> bytes:
        ...
        if usr_sesh.user_id > 0:
            profile_data = self.data.profile.get_profile_data(
                usr_sesh.user_id, self.version
            )
            rival_list = await self.data.profile.get_rivals(usr_sesh.user_id)
            rival_data = {"userRivalList": rival_list, "userId": usr_sesh.user_id}

            # Hay1tsme 01/09/2024: ??????????????????????????????????????????????????????????????
            rival_info = await OngekiBase.handle_get_user_rival_data_api_request(
                self, rival_data
            )
