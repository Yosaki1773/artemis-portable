            ...
            for dir in dirs:
                if os.path.exists(f"{root}/{dir}/Event.xml"):
                    with open(f"{root}/{dir}/Event.xml", "r", encoding="utf-8") as f:
                        ...
                        await self.data.static.put_event(
    ...
    async def read_reward(self, base_dir: str) -> None:
        ...
        for root, dirs, files in os.walk(base_dir):
            for dir in dirs:
                ...
                itemKind = OngekiConstants.REWARD_TYPES[
                    troot.find("ItemType").text
                ].value
                itemId = troot.find("RewardItem").find("ItemName").find("id").text

                await self.data.static.put_reward(
                    self.version, rewardId, rewardname, itemKind, itemId
                )
